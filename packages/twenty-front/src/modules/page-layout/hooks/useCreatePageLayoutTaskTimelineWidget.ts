...
import { createDefaultTaskTimelineWidget } from '@/page-layout/utils/createDefaultTaskTimelineWidget';
...
export const useCreatePageLayoutTaskTimelineWidget = ({
  ...
  const createPageLayoutTaskTimelineWidget = useCallback(
    (objectMetadataId?: string): PageLayoutWidget => {
      ...
      const sizes = WIDGET_SIZES[WidgetType.TASK_TIMELINE]!;
      const position = getDefaultWidgetPosition(
        store.get(pageLayoutDraggedAreaState),
        sizes.default,
        sizes.minimum,
      );
      const newWidget = createDefaultTaskTimelineWidget({
        id: widgetId,
  ...
  return { createPageLayoutTaskTimelineWidget };
};
